...
LONG InsLonZ, InsLatZ;	// 慣性航法経度&緯度[deg]x1e7
FLOAT InsCourse;		// INS方位[deg]
FLOAT InsCourseZ;		// INS方位[deg]
// マップマッチング関連
LONG MatchLon, MatchLat;// マップマッチ経度&緯度[deg]x1e7
FLOAT MatchCourse;		// マップマッチ[deg]
SHORT RxMatchCount;		// マップマッチ受信カウント
FLOAT MatchYawRate;		// マップマッチヨーレート(右が正)[deg/s]
BOOL fMapMatching;		// マップマッチ実行中
BOOL fRxMatch;			// マッチデータあり
LONG MatchLonErr, MatchLatErr, MatchCourseErr;
FLOAT GpsDoubtDist;		// GPSによる測位が疑わしい期間の走行距離[m]

#define CMDMAX 60		// コマンド受信受信バッファサイズ
CHAR CmdBuff[CMDMAX];	// コマンド受信バッファ
CHAR CmdPtr;			// コマンドバッファポインタ

SHORT TestMode;	// テストモード
...
FLOAT GetTrueYawRate(FLOAT* yofst, SHORT yawraw, FLOAT temp,
					...
					BOOL fcurve, BOOL fpark, FLOAT matchyaw,
					BOOL fstop, BOOL frun, BOOL fmatch);
...
void SprintN(const UCHAR* buf, SHORT len);
...
void setup()
{
	...
	#else
	Serial2.begin(19200);	// HILS Rx用に設定
	#endif //_HILS
...
#define MATCHFILT 1.0		// INS マップマッチ位置一致フィルタ定数
#define MATCHFILTCRS 0.2	// INS マップマッチ方位一致フィルタ定数
#define MATCHGODIST 300		// GPSが疑わしくてこの距離以上走行でマップマッチ
void Job100ms()
{
	static SHORT rtime, prktime, poserrcount, crserrcount;
	static FLOAT yawratez, accelyfilt, sgfilt, dist;
	FLOAT latcal, loncal, crscal, crsfilt;
	FLOAT poserr = 0.0, tofst = 0.0;
	FLOAT laterr = 0.0, lonerr = 0.0, crserr = 0.0;
...
LED_ON
	...
	if (AfterPowerOnTime <= 1)
	{
		...
		yawratez = accelyfilt = sgfilt = dist = 0.0;
	}
	...
	if (TestMode == 1)	// テストモード1の期間は
	{
		fGpsFix = fGpsDoubt = FALSE;	// GPSロスト状態とする
		GpsFixCount = 0;
	}
	// GPS疑わしい期間の走行距離[m]
	if (fGpsDoubt)
	{
		GpsDoubtDist += Vreal / 3.6 * Dt;
	}
	else
	{
		GpsDoubtDist = 0.0;
	...
	// マップマッチ実行条件
	// GPSロストまたはダウトで距離が進んだかつマッチデータを受信している
	fMapMatching = ((!fGpsFix || GpsDoubtDist >= MATCHGODIST) &&
					RxMatchCount >= 1);
	...
	YawRate = GetTrueYawRate(&YawOffset, YawRaw, Temperature, GpsYawRate, 
			Vreal, ArealFilt, fCurve, fPark, MatchYawRate,
			// 停止時オフセットキャンセル条件
			fStop,
			// 走行時オフセットキャンセル条件
			fGpsYawConfirm && !fCurve && fGpsRecv,
			// マップマッチ時オフセットキャンセル条件
			fMapMatching && !fCurve && Vreal >= YAWV);
bug1=YawOffset;
	...
	if (fRxMatch)
	{
		fRxMatch = FALSE;	// マップマッチ受信フラグクリア

		// マップマッチを実行するなら
		if (fMapMatching)
		{
			// INS位置補正量
			latcal = (FLOAT)MatchLatErr * MATCHFILT;
			loncal = (FLOAT)MatchLonErr * MATCHFILT;
			// INS位置をマップマッチ位置に徐々に一致させる
			InsLat += (LONG)latcal;
			InsLon += (LONG)loncal;

			// INS方位とマップマッチ方位の差分
			crserr = Normal180(MatchCourseErr);
			// INS方位補正量
			crscal = crserr * MATCHFILTCRS;
			// INS方位をマップマッチ方位に徐々に一致させる
	...
	if (fGpsRecv)
	{
		...
		{
			// INS方位とGPS方位の差分
			crserr = Normal180(Normal180(GpsCourse) -
									Normal180(InsCourse));
			if (abs(crserr) < CERR)
			{
				// INS方位補正量
	...
	while (Serial.available() > 0)	// 文字を受信していれば
	{
		...
		if (CmdPtr < CMDMAX - 2)	// バッファに余裕があるなら
		{
			...
			if (c == '\n')			// 行末か？
			{
				...
				// マップマッチングデータなら
				else if (strncmp(CmdBuff, "LON=", 4) == 0)
				{
					p = strtok(CmdBuff, ",");
					MatchLonErr = atol(p + 4);	// マップマッチ経度偏差
					p = strtok(NULL, ",");
					MatchLatErr = atol(p + 4);	// マップマッチ緯度偏差
					p = strtok(NULL, ",");
					MatchCourseErr = atof(p + 4);// マップマッチ方位偏差
					p = strtok(NULL, ",");
					fRxMatch = atoi(p);			// マッチデータあり
					MatchLon = InsLon + MatchLonErr;
					MatchLat = InsLat + MatchLatErr;
					MatchCourse =InsCourse + MatchCourseErr;
					if (!fRxMatch)	// マップマッチデータなしなら
					{
						MatchLonErr =  MatchLatErr = 0L;
						MatchCourseErr = 0.0;
						// F/SでINSの値を入れておく
						MatchLon = InsLon;
						MatchLat = InsLat;
						MatchCourse =InsCourse;
						fRxMatch = FALSE;
					}
					// マッチデータありの個数
					AddOnTime(fRxMatch, &RxMatchCount);
					CalcMatchYawRate();		// マップマッチヨーレート算出
				}
				else if (strncmp(CmdBuff, "TST=", 4) == 0)
				{
					TestMode = atoi(CmdBuff + 4);	// テストモード
				}
				CmdPtr = 0;	// ポインタを先頭に
			}
		}
...
/*----------------------------------------------------------------------------
	マップマッチヨーレート算出→MatchYawRate
----------------------------------------------------------------------------*/
void CalcMatchYawRate()
{
	static ULONG tim;
	static FLOAT dt, cdif;
	static FLOAT csz;
	...
	if (dt > 0.0 && RxMatchCount >= 2)
	{
		// マップマッチ方位の差分からマップマッチヨーレート[deg/s]を算出
		cdif = Normal180(Normal180(MatchCourse) - Normal180(csz));// 差分
		MatchYawRate = cdif / dt;
		csz = MatchCourse;	// 今回の方位をメモリ
	}
	else
	{
		MatchYawRate = 0.0;
...
void RxRMC()
{
	...
	AddOnTime(ffix, &fixcount);	// 連続確定回数
	fGpsFixRMC = BOOLSET(fixcount >= 2);
	if (ffix)	// 確定していたら
	{
		// バックなら負に
...
void TxGGA(BOOL fins)
{
	...
	if (fins)
	{
		...
		// マップマッチング中なら254.0
		hdop = fGpsDoubt ? (LONG)(GpsHdop * 100.0) : 100L;
		hdop = fMapMatching ? 25400L : hdop;
		strcat(Buff, F2Str(s, hdop, 0, 2));
	}
	else
	{
		// hdopをそのまま
					...
					BOOL fstop, BOOL frun, BOOL fmatch)
{
	...
	/*** マップマッチ時ヨーレートゼロ点補正 ***/
	if (fmatch && abs(matchyaw) <= CURVEYAW)
	{
		// ヨーレートセンサオフセットアップデート[deg/s]
		Filter(yofst, yawdelay - matchyaw, FILT_MATCHOFCAN);
	}
...
/*----------------------------------------------------------------------------
	Nバイトシリアル出力
	書式 void SprintN(const UCHAR* buf, SHORT len);
...
void MonitorOut()
{
	...
	Sprintfc(SatLevel, 1);
	// Status
	Sprintln(fMapMatching * 32 + fReverseRun * 16 +
		...
		if (BufPtr < BUFFMAX - 2)	// バッファに余裕があるなら
		{
			...
			if (c == '\n')		// 行末か？
			{
				...
				if (strncmp(Buff, "STX", 3) == 0)	//ネゴシエーション開始
				{
					...
					fReverseRun = FALSE;
					fStop = TRUE;
					fGpsConfirm = fMapMatching = FALSE;
					RxMatchCount = 0;
					Serial2.println("OK");
					BufPtr = 0;	// ポインタを先頭に
				}
				else if (fstx)		// 初期値受信
				{
					...
					YawOffset = atof(p);
	 				p = strtok(NULL, ",");
					InsLon = atol(p);
					InsLonZ = MatchLon = GpsLon = InsLon;
	 				p = strtok(NULL, ",");
					InsLat = atol(p);
					InsLatZ = MatchLat = GpsLat = InsLat;
	 				p = strtok(NULL, ",");
					InsCourse = atof(p);
					InsCourseZ = MatchCourse = GpsCourse = InsCourse;
	 				p = strtok(NULL, ",");
	 				VspConst = atol(p);
					BufPtr = 0;	// ポインタを先頭に
				}
...
void HILSoutNMEA()
{
	strcpy(StrGpsTime, "000000.00");
	strcpy(StrGpsDate, "000000");
	TxRMC(!fGpsFixGGA || fGpsDoubt);
	TxGGA(!fGpsFixGGA || fGpsDoubt);
}
#endif //_HILS
/*----------------------------------------------------------------------------
	16進文字を10進に変換
