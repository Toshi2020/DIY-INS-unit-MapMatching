...
*	rev1.2	11.Jan.2026 マップマッチング位置を表示
*
*****************************************************************************/
	...
	private void DrawMap() {
		...
		try {
		  ...
		  if ((Latz != mMapInfo.lat || Lonz != mMapInfo.lon ||
			...
			Location.distanceBetween(Latz, Lonz, mMapInfo.lat, mMapInfo.lon,
									result);
			// 距離変化が大きい？(初期位置から最初に緯度経度を受けた時)
			if (result[0] >= 1000.0d) {
				mMapController.setCenter(centerPoint);	// Mapを移動
			}
			// 頻度を抑える
			if (++cnt % 5 == 1) {

				mMapController.animateTo(centerPoint);	// Mapをスムース移動
			}
			...
			if (mMarker3.getPosition() != null) {// Matchマーカーがあれば
				mMarker3.remove(mMapView);	// Matchマーカーを削除する
			}
			...
			mMarker2.setIcon(getDrawable(
						R.drawable.ic_baseline_navigation_40_green));
			mMarker3.setIcon(getDrawable(
					mMapInfo.fmatching ?
						R.drawable.point_scan_24px2 :
						R.drawable.point_scan_24px));
			// マーカーの角度(右が負)
